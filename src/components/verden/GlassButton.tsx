...
export const GlassButton = ({ 
  ...
  const base = "font-display font-bold rounded-[24px] transition-all duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] active:scale-95 relative overflow-hidden flex items-center justify-center focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-background disabled:opacity-50 disabled:pointer-events-none";
  ...
  return (
    <motion.button
      ...
      whileTap={{ scale: internalStatus === "success" || props.disabled ? 1 : 0.96 }}
      disabled={props.disabled || internalStatus === "success"}
      onClick={(e) => {
        if (internalStatus === "success" || props.disabled) return;
